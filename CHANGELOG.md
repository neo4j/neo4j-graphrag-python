...
- Added `LexicalGraphBuilder.run_for_chunk()`, which builds the lexical graph for a single `TextChunk` — the `Chunk` node, the `Document` node when `document_info` is given, and the `FROM_DOCUMENT` and `NEXT_CHUNK` relationships — and returns it as a `Neo4jGraph`. Paired with `LexicalGraphBuilder.combine_graphs()`, which merges two graphs while deduplicating nodes by id and relationships by `(start_node_id, end_node_id, type)`, this makes it possible to build a lexical graph incrementally, one chunk at a time, instead of holding every chunk in memory. `run()` is unchanged and still takes the full `TextChunks`.
- Added `TextChunk.prev_chunk_id`, the uid of the preceding chunk in the document. `NEXT_CHUNK` is now created from this field alone, so a chunk carries everything needed to link it to its predecessor. `FixedSizeSplitter`, `LangChainTextSplitterAdapter` and `LlamaIndexTextSplitterAdapter` populate it as they yield chunks, and `LexicalGraphBuilder.run()` backfills it from document order when unset — so hand-built chunks, and custom splitters that do not set it, still get `NEXT_CHUNK` relationships. A custom splitter must set it to be usable with `run_for_chunk()`.
- Added `TextSplitter.iter_chunks()`, a lazy counterpart to `run()` that yields `TextChunk` objects one at a time instead of materializing a `TextChunks` result. `run()` is now defined in terms of it, and `FixedSizeSplitter`, `LangChainTextSplitterAdapter` and `LlamaIndexTextSplitterAdapter` all implement it; custom splitters should override `iter_chunks()` rather than `run()`.

### Changed

- (**breaking**) `LexicalGraphBuilder`'s node- and relationship-building helpers are now private, having previously been picked up by the API reference: `create_document_node`, `create_chunk_node`, `create_chunk_to_document_rel`, `create_next_chunk_relationship` and `create_node_to_chunk_rel` gained a leading underscore, and `process_chunk` was replaced by the new `run_for_chunk()`. They were only ever implementation details of `run()`; use `run()` or `run_for_chunk()` instead. `process_chunk_extracted_entities` is unchanged and remains public.
