...
def test_cypher_retriever_multiple_parameters(
    driver: neo4j.Driver, sample_data: str
) -> None:
    ...
    retriever = CypherRetriever(
        ...
        parameters={
            ...
            "min_stock": {"type": "integer", "description": "Minimum stock quantity"},
        },
    )

    # Execute the query with parameters
    result = retriever.search(
        parameters={"min_price": 5.0, "max_price": 15.0, "min_stock": 50}
    )
...
def test_cypher_retriever_optional_parameters(
    driver: neo4j.Driver, sample_data: str
) -> None:
...
def test_cypher_retriever_relationship_traversal(
    driver: neo4j.Driver, sample_data: str
) -> None:
...
def test_cypher_retriever_custom_formatter(
    driver: neo4j.Driver, sample_data: str
) -> None:
