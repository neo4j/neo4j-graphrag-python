...
from .probes import command_exists, ollama_models, probe_neo4j
...
def tier_2_local(assume_yes: bool, only: Optional[str]) -> None:
    heading(f"Tier 2: {TIER_NAMES[2]}")

    if not only or only == "ollama":
        _setup_ollama(assume_yes)

    if not only or only in {"weaviate", "qdrant", "pinecone"}:
        if confirm(
            "Start the local vector stores (Weaviate, Qdrant, Pinecone Local)?",
            True,
            assume_yes,
        ):
            if command_exists("docker"):
                ...
                for service in (
                    ...
                    _wait_for(service, attempts=20)
                print(
                    "  populate them before running the retrievers:\n"
    ...
    if not only or only == "sentence-transformers":
        if confirm(
            "Pre-download the all-MiniLM-L6-v2 embedding model?", True, assume_yes
        ):
            _warm_sentence_transformers()

    if not only or only == "spacy":
        if confirm(
            "Download the spaCy en_core_web_lg model (~560 MB)?", False, assume_yes
        ):
            run_command(
...
def _setup_ollama(assume_yes: bool) -> None:
    print()
    print(colour("-- Ollama (local, free)", BOLD))
    if not command_exists("ollama"):
        print("   ollama is not installed.")
        print("   native:  brew install ollama && ollama serve")
        print(
            "   docker:  docker compose -f examples/docker-compose.yml "
            "--profile ollama up -d"
        )
        if command_exists("brew") and confirm(
            "   Install it with Homebrew now?", True, assume_yes
        ):
            run_command(["brew", "install", "ollama"])
        else:
            return

    if not reqs.service_available(reqs.OLLAMA_SERVER):
        print(
            colour(
                "   the ollama server is not running - start it with: ollama serve",
                YELLOW,
            )
        )
        return
    ...
    for model in (OLLAMA_CHAT_MODEL, OLLAMA_EMBED_MODEL):
        if any(name.startswith(model.split(":")[0]) for name in pulled):
