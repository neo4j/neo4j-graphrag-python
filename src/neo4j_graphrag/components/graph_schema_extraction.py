...
Pipeline code uses :class:`~neo4j_graphrag.components.schema.GraphSchema`
exclusively; convert via :meth:`~neo4j_graphrag.components.schema.GraphSchema.from_extraction_output`.
...
sentinels where runtime :class:`~neo4j_graphrag.components.schema.ConstraintType`
uses ``None``).
"""
...
class ExtractedPropertyType(BaseModel):
    """Property definition aligned with :class:`~neo4j_graphrag.components.schema.PropertyType` for extraction."""
...
class ExtractedConstraintType(BaseModel):
    ...
    :class:`~neo4j_graphrag.components.schema.ConstraintType`; those are enforced
    when building :class:`~neo4j_graphrag.components.schema.GraphSchema`, not here.
    """
...
def wire_extraction_constraints_for_graph_schema(
    ...
    """Map extraction constraint dicts to values compatible with :class:`~neo4j_graphrag.components.schema.ConstraintType`.

    The empty-string ``relationship_type`` wire sentinel (and any missing or ``None`` value) is removed,
    so that :class:`~neo4j_graphrag.components.schema.ConstraintType` falls back to its
