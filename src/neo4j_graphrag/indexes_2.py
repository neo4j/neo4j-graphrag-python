...
# from .types import FulltextIndexModel, IndexType, VectorIndexModel
...
#     neo4j_database: Optional[str] = None,
#     entity_type: IndexType = IndexType.NODE,
# ) -> None:
#     if entity_type == IndexType.NODE:
#         match_entity = "(o)"
#         procedure = "setNodeVectorProperty"
#     elif entity_type == IndexType.RELATIONSHIP:
#         match_entity = "()-[o]->()"
#         procedure = "setRelationshipVectorProperty"
#     else:
...
#     if not all(len(embeddings) == len(embedding[0]) for embedding in embeddings):
#         raise ValueError("All embeddings must be of the same size")
#     try:
#         query = (
#             "UNWIND $rows AS row "
#             f"MATCH {match_entity} "
#             "WHERE elementId(o) = row.id "
#             "WITH o "
#             f"CALL db.create.{procedure}(o, $embedding_property, row.embedding) "
#             "RETURN o"
#         )
#         parameters = {
#             "rows": [
...
# ) -> List[str]:
#     """
#     Utility function for batch upserting texts and embeddings to a set of nodes.

#     Merges or creates nodes labeled `node_label` with the specified text and embedding
#     properties. Optionally attaches metadata for each node. If no IDs are provided, a
#     unique ID is generated from the MD5 hash of the text.

#     Args:
#         driver (neo4j.Driver): The Neo4j driver instance used to run the query.
#         node_label (str): The label of the nodes to be created or merged.
#         text_property (str): The node property where text data will be stored.
#         embedding_property (str): The node property where embedding vectors will be stored.
#         texts (Iterable[str]): An iterable of text strings to be upserted.
...
#             determines which syntax is used in the query.
#         metadatas (Optional[List[dict]]): Optional list of metadata dictionaries, one per text.
#         ids (Optional[List[str]]): Optional list of IDs to assign to the nodes. If omitted,
#             IDs are generated by hashing the text.

#     Returns:
#         List[str]: A list of IDs corresponding to the upserted nodes.
#     """
#     if ids is None:
#         ids = [md5(text.encode("utf-8")).hexdigest() for text in texts]
#     if not metadatas:
#         metadatas = [{} for _ in texts]
#     import_query = _get_upsert_texts_and_embeddings_query(
#         node_label=node_label,
#         text_property=text_property,
#         embedding_property=embedding_property,
#         neo4j_version_is_5_23_or_above=neo4j_version_is_5_23_or_above,
#     )
#     parameters = {
#         "data": [
...
#         session.run(neo4j.Query(text=import_query), parameters)
#     return ids
...
#         embeddings=[vector],
#         neo4j_database=neo4j_database,
#         entity_type=IndexType.RELATIONSHIP,
#     )
